...
class No_Jetpack_Carousel {
	...
	function init() {
		...
		if ( is_admin() ) {
			...
			add_filter( 'post_gallery', array( $this, 'post_gallery' ), 1000, 2 ); // load later than other callbacks hooked it
    ...
    function post_gallery( $output, $attr ) {
        ...
        if ( !empty($include) ) {
            ...
            $_attachments = array_merge( $_attachments, get_posts( array('include' => $include, 'post_status' => 'inherit', 'post_type' => 'attachment', 'post_mime_type' => 'video', 'order' => $order, 'orderby' => $orderby) ) );
            ...
            $attachments = array_merge( $attachments, get_children( array('post_parent' => $id, 'exclude' => $exclude, 'post_status' => 'inherit', 'post_type' => 'attachment', 'post_mime_type' => 'video', 'order' => $order, 'orderby' => $orderby) ) );

        } else {

            $attachments = get_children( array('post_parent' => $id, 'post_status' => 'inherit', 'post_type' => 'attachment', 'post_mime_type' => 'image', 'order' => $order, 'orderby' => $orderby) );
            $attachments = array_merge( $attachments, get_children( array('post_parent' => $id, 'post_status' => 'inherit', 'post_type' => 'attachment', 'post_mime_type' => 'video', 'order' => $order, 'orderby' => $orderby) ) );
